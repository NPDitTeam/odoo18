    ...
    def run(self):
        ...
        if self.existing_or_new == 'existing':
            ...
            try:
                ...
                _logger.error("Google API connection failed: %s", e)
                raise UserError(_("Cannot connect to Google Sheets API. Please check the server's internet connection and try again.\n\nError: %s") % e)
            try:
                self.sync_spreadsheet()
            except (TimeoutError, ConnectionError, OSError) as e:
    ...
    def sync_spreadsheet_every_x_times(self):
        for spreadsheet in self.env['connect.spreadsheet'].search(
            ...
            try:
                ...
                _logger.warning("Cron sync skipped for '%s' (ID: %s) - Google API timeout: %s",
                                spreadsheet.spreadsheet_title, spreadsheet.id, e)
            except Exception as e:
                _logger.error("Cron sync failed for '%s' (ID: %s): %s",
                              spreadsheet.spreadsheet_title, spreadsheet.id, e)
