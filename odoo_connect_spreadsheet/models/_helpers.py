...
GOOGLE_API_TIMEOUT = 10  # seconds per attempt (short, because we retry)
MAX_RETRIES = 3

# Force IPv4 only — workaround for servers with broken IPv6 routing.
_original_getaddrinfo = socket.getaddrinfo


def _ipv4_getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
    results = _original_getaddrinfo(host, port, socket.AF_INET, type, proto, flags)
    random.shuffle(results)  # Shuffle IP order to avoid always hitting the same bad IP
    return results
...
class RetryHttp(httplib2.Http):
    """httplib2.Http with automatic retry on connection timeout."""
    ...
    def request(self, uri, method="GET", body=None, headers=None,
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                last_error = e
                _logger.warning("Google API attempt %d/%d failed (%s): %s",
                                attempt + 1, MAX_RETRIES, uri, e)
                if attempt < MAX_RETRIES - 1:
                    self.connections.clear()  # Close stale connections, force new TCP on retry
                    time.sleep(1)
        raise last_error


def _build_authorized_http(credentials, timeout=GOOGLE_API_TIMEOUT):
    """Build an authorized HTTP client with timeout and retry."""
    http = RetryHttp(timeout=timeout)
...
def google_drive_authentication(scopes, service_account_json_credentials):
    ...
    drive_service = discovery.build('drive', 'v3', http=authorized_http)
...
def google_sheet_authentication(scopes, service_account_json_credentials):
    credentials = service_account.Credentials.from_service_account_info(service_account_json_credentials, scopes=scopes)
    authorized_http = _build_authorized_http(credentials)
    spreadsheet_service = discovery.build('sheets', 'v4', http=authorized_http)
