...
class JasperReport:
    ...
    def extract_fields(self, field_tags, ns, jr7=False):
        ...
        for tag in field_tags:
            name = tag.get('name')
            type = tag.get('class')
            # JR 6.x uses <fieldDescription> with namespace
            # JR 7.x uses <description> without namespace
            if jr7:
                path = (tag.findtext('description', '') or '').strip()
            else:
                path = tag.findtext('{%s}fieldDescription' % ns, '').strip()
                if not path:
                    # Fallback: also try JR 7.x <description> tag
                    path = (tag.findtext('description', '') or '').strip()
    ...
    def extract_properties(self):
        ...
        # Detect JR format version:
        # JR 6.x uses xmlns="http://jasperreports.sourceforge.net/jasperreports"
        # JR 7.x has no default namespace, uses 'query' instead of
        # 'queryString', 'description' instead of 'fieldDescription', etc.
        ...
        if default_ns:
            ...
            # JR 7.x mode (no namespace)
            ns = ''
            nss = {}
            jr7 = True

        # Helper: execute XPath for both JR 6.x (namespaced) and 7.x (no ns)
        def xp(node, path):
            if jr7:
                return node.xpath(path.replace('jr:', ''))
            return node.xpath(path, namespaces=nss)

        # Helper: findtext for child element (handles namespace)
        def ft(element, tag):
            if jr7:
                return element.findtext(tag, '')
            return element.findtext('{%s}%s' % (ns, tag), '')

        # Language
        # JR 6.x: <queryString language="xPath">
        # JR 7.x: <query language="xPath">
        lang_tags = xp(doc, '/jr:jasperReport/jr:queryString')
        if not lang_tags:
            lang_tags = xp(doc, '/jr:jasperReport/jr:query')
        if lang_tags:
        ...
        copies_field_tags = xp(doc, path1)
        ...
        header_tags = xp(doc, path3)
        if header_tags and 'value' in header_tags[0].keys():
            self.is_header = True

        field_tags = xp(doc, '/jr:jasperReport/jr:field')
        self.fields, self.field_names = self.extract_fields(
            field_tags, ns, jr7)
        ...
        subreport_tags = xp(doc, '//jr:subreport')

        for tag in subreport_tags:
            data_source_expression = ft(tag, 'dataSourceExpression')
            ...
            subreport_expression = ft(tag, 'subreportExpression')
            if not subreport_expression and jr7:
                # JR 7.x may use 'expression' instead of 'subreportExpression'
                subreport_expression = ft(tag, 'expression')
        ...
        dataset_tags = xp(doc, '//jr:datasetRun')
        ...
        for tag in dataset_tags:
            ...
            copies_field_tags = xp(tag, path9)
            if copies_field_tags and 'value' in copies_field_tags[0].keys():
                copies_field = \
                    self.path_prefix + copies_field_tags[0].get('value')
            ...
            path_prefix_tags = xp(tag, path13)
            ...
            sub_dataset = xp(doc, path14 % sub_dataset_name)[0]
            field_tags = xp(sub_dataset, 'jr:field')
            fields, field_names = self.extract_fields(
                field_tags, ns, jr7)
