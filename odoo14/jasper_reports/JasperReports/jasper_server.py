...
class JasperServer:
    ...
    def start(self):
        ...
        # Resolve the full path to the java executable.
        # Use the configured java_path + /bin/java if available,
        # otherwise fall back to 'java' from PATH.
        java_bin = 'java'
        if java_path:
            candidate = os.path.join(str(java_path), 'bin', 'java')
            if os.name == 'nt':
                candidate += '.exe'
        ...
        command = [java_bin, '-Djava.awt.headless=true',
                   '-XX:MaxHeapSize=512m',
