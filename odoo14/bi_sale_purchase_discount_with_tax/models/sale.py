    ...
    def _amount_all(self):
        ...
        for order in self:
            ...
            for line in order.order_line:
                ...
                amount_price_subtotal_without_discount += line.price_subtotal_without_discount  # (line.product_uom_qty*line.price_unit)
                    ...
                    if line.price_unit and line.product_uom_qty:
                        if line.discount_amount:
                            ...
                            if line.tax_id:
                                ...
                                stwd = 0.0
                                stwd = (line.price_unit * line.product_uom_qty) * (100 / (100 + taxes_amount_include))

                                discount_percent = (line.discount_amount / (line.product_uom_qty * stwd)) * 100

                                # discount_percent = (line.discount_amount/((line.product_uom_qty * line.price_unit)))*100
                                price_unit_discount = line.price_unit * (1 - (discount_percent or 0.0) / 100.0)
                                taxes_res = line.tax_id.compute_all(price_unit_discount, cur_obj, line.product_uom_qty,
                                                                    line.product_id)
                                ...
                                line.price_subtotal = (line.product_uom_qty * line.price_unit) - line.discount_amount
                                line.price_subtotal_without_discount = (line.product_uom_qty * line.price_unit)
                                line.price_tax = 0
...
# SaleAdvancePaymentInv._create_invoice removed - method doesn't exist in Odoo 18
                    ...
                    if line.discount_type == 'line':
                        if line.discount_method == 'fix':
                            ...
                            if line.tax_id:
                                if line.discount_amount:
                                    if line.tax_id:
                                        if line.price_unit and line.product_uom_qty:
                                            ...
                                            stwd = 0.0
                                            stwd = (line.price_unit * line.product_uom_qty) * (
                                ...
                                # price = (vals['price_unit'] * vals['product_uom_qty']) - line.discount_amount
                                ...
                                for taxes_id in line.tax_id:
                                    if taxes_id.price_include:
                                        ...
                                        if line.discount_amount > 0:
                                            line.discount_amt = (line.price_unit * line.product_uom_qty) * (
                                        ...
                                        if line.discount_amount > 0:
                                            line.discount_amt = (
                                                                        line.product_uom_qty * line.price_unit) * line.discount_amount / 100

                            stwd = 0.0
                            stwd = (line.price_unit * line.product_uom_qty) * (100 / (100 + taxes_amount_include))
                            price = (line.price_unit * line.product_uom_qty) * (1 - (line.discount_amount or 0.0) / 100.0)
                            # price_x = ((vals['price_unit'] * vals['product_uom_qty'])-((vals['price_unit'] * vals['product_uom_qty']) * (1 - (line.discount_amount or 0.0) / 100.0)))
                            taxes = line.tax_id.compute_all(price, cur_obj, 1, line.product_id)
                            if line.discount_amount > 0:
                                line.discount_amt = (line.product_uom_qty * line.price_unit) * line.discount_amount / 100
                            line.price_subtotal_without_discount = stwd
                            line.update({
                                'price_tax': sum(t.get('amount', 0.0) for t in taxes.get('taxes', [])),
                            ...
                            if line.tax_id:
                                for taxes_id in line.tax_id:
                                    if taxes_id.price_include:
                                        ...
                                        if taxes_id.amount > 0:
                                            taxes_amount_execlude = taxes_id.amount
                            taxes_res = line.tax_id.compute_all(line.price_unit, cur_obj, line.product_uom_qty,
                                                                line.product_id)
                            stwd = 0.0
                            stwd = (line.price_unit * line.product_uom_qty) * (100 / (100 + taxes_amount_include))
                            line.price_subtotal_without_discount = stwd
                            line.price_tax = sum(t.get('amount', 0.00000) for t in taxes_res.get('taxes', []))
                            taxes = line.tax_id.compute_all(line.price_unit, cur_obj, line.product_uom_qty, line.product_id)
                            line.update({
                                'price_tax': sum(t.get('amount', 0.0) for t in taxes.get('taxes', [])),
                                'price_total': taxes['total_included'],
                                'price_subtotal': taxes['total_excluded'],
    ...
    def _prepare_invoice_line(self, **optional_values):
        ...
        if line.discount_method == 'fix':
            if line.tax_id:
                ...
                stwd = 0.0
                stwd = (line.price_unit * line.product_uom_qty) * (100 / (100 + taxes_amount_include))
                if (line.product_uom_qty * stwd) > 0 :
                    discount_percent = (line.discount_amount / (line.product_uom_qty * stwd)) * 100
        elif line.discount_method == 'per':
            discount_percent = line.discount_amount
        else:
            if (line.product_uom_qty * line.price_unit) > 0:
                discount_percent = line.discount_amount / ((line.product_uom_qty * line.price_unit) * 100)
...
class ResCompany(models.Model):
    ...
    purchase_account_id = fields.Many2one('account.account', domain=[('account_type', '=', 'income'),
