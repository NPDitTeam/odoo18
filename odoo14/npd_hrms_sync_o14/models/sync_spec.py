...
SYNC_SPECS = [
    ...
    },
    {
        ...
        # ใบที่ถูกลบฝั่ง 14 ให้ลบตามด้วย ไม่งั้นค้างฝั่ง 18 ตลอดไปแล้วยอดเกินจริง
        # ทำเฉพาะหัวข้อนี้และเฉพาะงวดที่เพิ่งดึงมา ตามที่ตกลงกันไว้
        'sync_deletes': True,
    ...
    },
    {
        ...
        # ฝั่ง 18 ไม่มีช่องชื่อนี้ ตัวเทียบช่องอัตโนมัติจึงไม่ขอมาให้
        # แต่ตัวแปลงต้องใช้ เพราะมันคือธง "หักเงินงวดนี้ไปแล้ว" ตัวจริง
        'read_extra': ['is_synced'],
    },
    {
