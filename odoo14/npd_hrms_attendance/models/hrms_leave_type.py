...
class HrmsLeaveBalance(models.Model):
    ...
    _sql_constraints = [
        ...
        # กันติดลบถึงระดับฐานข้อมูล ไม่ใช่แค่ในโค้ดที่หักสิทธิ์
        # เผื่อมีทางอื่นมาเขียนค่าตรง ๆ ในอนาคต (สคริปต์ยกข้อมูล งานตั้งเวลา
        # หรือการแก้จากหน้าจอ) จะได้ไม่หลุดเป็นเลขติดลบอีก
        ('remaining_not_negative', 'check(remaining >= 0)',
         'วันลาคงเหลือติดลบไม่ได้ ต่ำสุดคือศูนย์'),
    ]
    ...
    def _deduct(self, days):
        ...
        # ถ้าบล็อกไว้ ประวัติการลาจะขาด จึงปล่อยให้ยกมาได้
        ...
        # คงเหลือต่ำสุดคือศูนย์ ห้ามติดลบ
        #
        # ตัวเลขติดลบไม่มีความหมายในทางปฏิบัติ — พอขึ้นปีใหม่ระบบจะรีเซ็ต
        # ให้เท่าสิทธิ์ที่ตั้งไว้ต่อปีอยู่แล้ว ยอดติดลบจึงไม่ได้ถูกยกไปหักต่อ
        # แต่ระหว่างปีมันไปโผล่ในแอปและในข้อความเตือน ทำให้คนอ่านสับสน
        self.remaining = max(0.0, self.remaining - days)
